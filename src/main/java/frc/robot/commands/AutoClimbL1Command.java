...
public class AutoClimbL1Command extends CTSequentialCommandGroup {
    ...
    public AutoClimbL1Command(Drive drive, Climber climber, Turret turret, Intake intake) {
        this.addCommands(
            ...
            new InstantCommand((() -> intake.setIntakeAngle(IntakeAngle.HOME)), intake)
            ...
            drive.run(() -> drive.runVelocity(new ChassisSpeeds(-0.15, 0, 0)))
                 .withTimeout(1.75)
                 .asProxy(),
                 
            drive.run(() -> drive.runVelocity(new ChassisSpeeds(0.0, 0.1, 0)))
                 .until(climber::isReadyToClimb)
                 .asProxy(),
            // Little oomph just incase the switch is early activated
            // drive.run(() -> drive.runVelocity(new ChassisSpeeds(-0.025, 0, 0)))
            //      .withTimeout(0.1)
            //      .asProxy(),   
